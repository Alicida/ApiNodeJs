...
function pruebas(req,res){
	res.status(200).send({message: 'Método pruebas del controlador.'});
}
...
function getSongs(req,res){
	...
	if(!albumId){
		//Sacar todos las canciones
		var find = Song.find({}).sort('name');
	}else{
		//Sacar las canciones de un album concreto de la base de datos
...
function updateSong(req, res){
	...
	Song.findByIdAndUpdate(songId, update, (err, songUpdated) => {
		if(err){
			res.status(500).send({message: 'Error al actualizar el canción'});
		}else{
			if(!songUpdated){
...
function uploadFile(req, res){
	...
	if(req.files){
		...
		res.status(200).send({message: 'No se ha subido el archivo de audio'});
	}
}
